...
describe('grafo ramificato', () => {
  it('valida il grafo obbligatorio 50/50 con tre rami e ricongiungimento', () => {
    const graph = createDemoProject().graph;
  ...
  it('segnala una somma diversa da cento', () => { const graph = createDemoProject().graph; graph.edges.find(edge => edge.id === 'e1')!.probability = 60; expect(validateGraph(graph).some(issue => issue.message.includes('invece di 100'))).toBe(true); });
});

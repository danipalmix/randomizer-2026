...
export type VisualGroup = { id: string; name: string; color: string; nodeIds: string[]; x: number; y: number; width: number; height: number };
...
export type Project = {
  version: 4; graph: Graph; visualGroups: VisualGroup[]; migrationWarnings?: string[]; name: string; description: string; symbol: string; externalUrl: string; size: number;
...
export const emptyGraph = (): Graph => ({ nodes: [{ id: 'start', type: 'start', x: 80, y: 260 }, { id: 'end', type: 'end', x: 760, y: 260 }], edges: [{ id: 'initial-edge', source: 'start', target: 'end', probability: 100 }] });
export const newProject = (): Project => ({
  version: 4, graph: emptyGraph(), visualGroups: [], name: 'Nebula Atelier', description: 'Una collezione originale creata offline.', symbol: 'NEB', externalUrl: '', size: 25,

...
ipcMain.handle('project:open',async()=>{const r=await dialog.showOpenDialog({filters:[{name:'Progetto LayerForge',extensions:['layerforge','json']}],properties:['openFile']});if(r.canceled)return null;const file=r.filePaths[0];roots.add(path.dirname(file));const data=JSON.parse(await fs.readFile(file,'utf8'));if((data.version??1)<4)await fs.copyFile(file,`${file}.pre-v4.backup`);return{file,data}});
